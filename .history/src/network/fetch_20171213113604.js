...
function wrappingFetch({ url, method, data }, optionsArg) {
    ...
    // Select if the request should be rate-limited or not
    // Only GET method should be for now (CSRF problem)
    const sendFunc = noRateLimiter || !rateLimiter.enableRateLimiter || method.toUpperCase().trim() !== 'GET' ? fetch : getRateLimitedFetch();
    // Do the request
